...
macro_rules! not_impl {
    () => { anyhow::bail!("MongoDB store not yet implemented") };
}

#[async_trait]
impl DataStore for MongoStore {
    async fn ensure_schema(&self) -> Result<()> { not_impl!() }

    async fn list_flags(&self) -> Result<Vec<crate::flag::CellFlag>> { not_impl!() }
    async fn upsert_flag(&self, _key: &str, _color: &str) -> Result<crate::flag::CellFlag> { not_impl!() }
    async fn delete_flag(&self, _key: &str) -> Result<()> { not_impl!() }

    async fn list_remarks(&self) -> Result<Vec<crate::remark::Remark>> { not_impl!() }
    async fn upsert_remark(&self, _id: &str, _body: &str, _kind: &str, _is_private: bool, _resolved_at: Option<chrono::DateTime<chrono::Utc>>, _targets: &[crate::remark::RemarkTarget]) -> Result<crate::remark::Remark> { not_impl!() }
