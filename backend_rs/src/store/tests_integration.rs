...
fn db_urls() -> Vec<String> {
    ...
    if let Ok(raw) = std::env::var("DB_URLS") {
        return raw.split(',').map(str::trim).filter(|s| !s.is_empty()).map(str::to_owned).collect();
    }
...
fn surreal_url() -> Option<String> {
    db_urls().into_iter().find(|u| u.starts_with("surreal") || u.starts_with("wss://") || u.starts_with("ws://"))
}
...
async fn make_store() -> Option<Arc<dyn DataStore>> {
    ...
    use_test_surreal_db();
    let urls = db_urls();
    if urls.is_empty() { eprintln!("skip: no DB URLs configured (DB_URLS / DATABASE_URL)"); return None; }
    let url_refs: Vec<&str> = urls.iter().map(String::as_str).collect();
    match open_all(&url_refs).await {
        Ok(s) => Some(s),
        Err(e) => { eprintln!("Failed to open store: {e}"); None }
    }
}
...
macro_rules! setup {
    () => {{
        let pg = pg_url();
        if let Some(ref u) = pg { pg_cleanup(u).await; }
        surreal_cleanup().await;
        match make_store().await { Some(s) => s, None => return }
    }};
}
...
async fn integration_create_and_read_row() {
    ...
    let row = store.create_row(table_id, row_id, Some("Integration test row"), None)
        .await.expect("create_row failed");
    assert_eq!(row.id, row_id);
    assert_eq!(row.table_id, table_id);

    let page = store.list_data_rows(table_id, &RowQuery { page: 1, per_page: 50, ..Default::default() })
        .await.expect("list_data_rows failed");
...
async fn integration_create_row_idempotent() {
    ...
    store.create_row(table_id, row_id, Some("Second"), None).await.unwrap(); // same id

    let page = store.list_data_rows(table_id, &RowQuery { page: 1, per_page: 50, ..Default::default() })
        .await.unwrap();
    let matching: Vec<_> = page.data.iter().filter(|r| r["id"] == row_id).collect();
    assert_eq!(matching.len(), 1, "idempotent create produced duplicate rows");
}
...
async fn integration_patch_row_value_reflects() {
    ...
    store.patch_row_value(table_id, row_id, "status", serde_json::json!("verified"))
        .await.expect("patch_row_value failed");
...
async fn integration_upsert_rows_batch_idempotent() {
    ...
    store.upsert_rows_batch(table_id, &rows_updated).await.unwrap();
...
async fn integration_remark_lifecycle() {
    ...
    let remark = store.upsert_remark(id, "Initial body", "note", false, None, &[])
        .await.expect("upsert_remark failed");
    ...
    let found = remarks.iter().find(|r| r.id == id).expect("remark not found after update");
    assert_eq!(found.body, "Updated body");
    assert_eq!(found.kind, "comment");
    assert!(found.is_private);

    store.delete_remark(id).await.unwrap();
    assert!(!store.list_remarks().await.unwrap().iter().any(|r| r.id == id),
...
async fn integration_remark_with_targets() {
    ...
    let targets = vec![
        crate::remark::RemarkTarget { row_id: "_test_row_1".into(), column_id: "stars".into() },
        crate::remark::RemarkTarget { row_id: "_test_row_2".into(), column_id: "forks".into() },
    ];

    store.upsert_remark(id, "Multi-target remark", "note", false, None, &targets).await.unwrap();

    let remarks = store.list_remarks().await.unwrap();
    let found = remarks.iter().find(|r| r.id == id).expect("remark not found");
...
async fn integration_remark_resolve() {
    ...
    let found = store.list_remarks().await.unwrap().into_iter().find(|r| r.id == id).unwrap();
...
async fn integration_hidden_row_add_list_remove() {
    ...
    assert!(store.list_hidden_rows().await.unwrap().iter().any(|r| r.row_id == row_id),
...
async fn integration_hidden_row_idempotent() {
    ...
    let rows: Vec<_> = store.list_hidden_rows().await.unwrap().into_iter()
        .filter(|r| r.row_id == row_id).collect();
...
async fn integration_hidden_column_add_list_remove() {
    ...
    assert!(store.list_hidden_columns().await.unwrap().iter().any(|c| c.column_id == col_id),
...
async fn integration_table_create_list_patch_delete() {
    ...
    assert!(store.list_tables().await.unwrap().iter().any(|t| t.id == id),
...
async fn integration_table_create_idempotent() {
    ...
    store.create_table(id, "Second Title", None, None).await.unwrap(); // same id — must not error or duplicate

    let tables: Vec<_> = store.list_tables().await.unwrap().into_iter()
        .filter(|t| t.id == id).collect();
    assert_eq!(tables.len(), 1, "idempotent create produced duplicates");
}

// ── Custom column tests ───────────────────────────────────────────────────────

fn text_col(name: &str) -> crate::custom_column::CustomColumnInput {
...
async fn integration_custom_column_upsert_and_list() {
    ...
    let col = store.upsert_custom_column(table_id, "_test_col_name", &text_col("name"))
        .await.unwrap();
    assert_eq!(col.id, "_test_col_name");
    assert_eq!(col.name, "name");

    let cols = store.list_custom_columns(table_id).await.unwrap();
    assert!(cols.iter().any(|c| c.id == "_test_col_name"), "column not in list");
}
...
async fn integration_custom_column_group_with_sub_columns() {
    ...
    store.upsert_custom_column(table_id, "_test_col_owner", &group_input).await.unwrap();

    // Create sub-column with parent_ids pointing to the group
    let sub_input = crate::custom_column::CustomColumnInput {
        name: "owner_login".to_owned(),
        label: Some("Login".to_owned()),
        parent_ids: vec!["_test_col_owner".to_owned()],
        types: Some(vec!["text".to_owned()]),
        ..Default::default()
    };
    store.upsert_custom_column(table_id, "_test_col_owner_login", &sub_input).await.unwrap();

    let cols = store.list_custom_columns(table_id).await.unwrap();
    let group = cols.iter().find(|c| c.id == "_test_col_owner").expect("group col missing");
    let sub   = cols.iter().find(|c| c.id == "_test_col_owner_login").expect("sub col missing");
...
async fn integration_custom_column_delete() {
    ...
    store.upsert_custom_column(table_id, "_test_col_del", &text_col("to_delete")).await.unwrap();

    assert!(store.list_custom_columns(table_id).await.unwrap().iter()
        .any(|c| c.id == "_test_col_del"));

    store.delete_custom_column("_test_col_del").await.unwrap();
    assert!(!store.list_custom_columns(table_id).await.unwrap().iter()
        .any(|c| c.id == "_test_col_del"), "column still present after delete");
}
...
async fn integration_custom_column_freeze() {
    ...
    store.upsert_custom_column(table_id, "_test_col_freeze", &text_col("freezable")).await.unwrap();

    let col = store.set_table_column_frozen(table_id, "_test_col_freeze", true).await.unwrap();
    assert!(col.is_frozen);

    let col = store.set_table_column_frozen(table_id, "_test_col_freeze", false).await.unwrap();
    assert!(!col.is_frozen);
}
...
async fn integration_custom_column_full_metadata_round_trips() {
    ...
    let col = store.upsert_custom_column(table_id, "_test_col_stars", &input).await.unwrap();
    assert_eq!(col.label.as_deref(), Some("Stars ⭐"));
...
async fn integration_ops_log_applied_at_set_after_write() {
    let pg = match pg_url() { Some(u) => u, None => { eprintln!("skip"); return; } };
    pg_cleanup(&pg).await;
    surreal_cleanup().await;
    let store = match make_store().await { Some(s) => s, None => return };

    store.upsert_flag(&nanoid::nanoid!(), "_test_flag_ops_applied", "blue").await.unwrap();
...
async fn integration_ops_log_no_pending_after_all_writes_succeed() {
    ...
    store.upsert_flag(&nanoid::nanoid!(), "_test_flag_nopending", "blue").await.unwrap();
    store.create_row("_test_nopending", "_test_row_nopending", None, None).await.unwrap();
    ...
    let ours: Vec<_> = pending.iter()
        ...
        .collect();
    assert!(ours.is_empty(), "found {n} unexpected pending ops: {ours:?}", n = ours.len());
}
...
async fn integration_ops_log_full_payload_for_remark() {
    ...
    let targets = vec![
        crate::remark::RemarkTarget { row_id: "row1".into(), column_id: "stars".into() },
    ];
    store.upsert_remark("_test_remark_payload", "Important body", "note", false, None, &targets)
        .await.unwrap();
...
async fn live_db_per_request_timeout_override() {
    let store = setup!();
    store.create_row("_test_timeout_override", "_test_row_to_override", Some("Timeout test"), None)
        .await.unwrap();

    let page = store.list_data_rows("_test_timeout_override", &RowQuery {
        page: 1, per_page: 50, read_timeout_secs: Some(30), ..Default::default()
    }).await.unwrap();
    assert!(page.total >= 1);
}
...
async fn integration_upsert_github_repos_no_duplicates() {
    ...
    let updated = serde_json::json!({ "github_id": -999001, "name": "_test_/repo-dedup", "stars": 99 });
