...
type SubscribeStream = Pin<Box<dyn futures_core::Stream<Item = Result<ServerEvent, Status>> + Send + 'static>>;

// ── Conversion helpers ─────────────────────────────────────────────────────────

fn flag_to_proto(f: &flag::CellFlag) -> Flag {
    Flag { id: f.id.clone(), key: f.key.clone(), color: f.color.clone() }
}
    ...
    async fn dispatch(
        ...
        match payload {
            OpPayload::UpsertFlag(p) => {
                let f = self.state.store.upsert_flag(&p.key, &p.color).await?;
                self.state.store.append_ops_log("flag.upsert", serde_json::json!({
                    "op_id": op_id, "key": f.key, "color": f.color,
                }), None).await;
            ...
            OpPayload::DeleteFlag(p) => {
                ...
                self.state.store.append_ops_log("flag.delete", serde_json::json!({
                    "op_id": op_id, "key": p.key,
                }), None).await;
                let event = flag_event(EventKind::Delete, Flag {
                    id: String::new(), key: p.key, color: String::new(),
                });
                Ok((event, vec![]))
            }
            OpPayload::UpsertRemark(p) => {
                ...
                let r = self.state.store
                    ...
                    .await?;
                self.state.store.append_ops_log("remark.upsert", serde_json::json!({
            ...
            OpPayload::DeleteRemark(p) => {
                ...
                self.state.store.append_ops_log("remark.delete", serde_json::json!({
                    "op_id": op_id, "id": p.id,
                }), None).await;
                let event = remark_event(EventKind::Delete, Remark { id: p.id, ..Default::default() });
                Ok((event, vec![]))
            }
            ...
            OpPayload::AddHiddenCol(p) => {
                let col = self.state.store.add_hidden_column(&p.column_id).await?;
                let event = hidden_col_event(EventKind::Upsert, HiddenCol {
                    id: col.id.clone(), column_id: p.column_id,
                });
                Ok((event, serde_json::to_vec(&serde_json::json!({ "id": col.id }))?))
            }
            OpPayload::RemoveHiddenCol(p) => {
                self.state.store.remove_hidden_column(&p.column_id).await?;
                let event = hidden_col_event(EventKind::Delete, HiddenCol {
                    id: String::new(), column_id: p.column_id,
                });
                Ok((event, vec![]))
            }
            ...
            OpPayload::DeleteCustomCol(p) => {
                ...
                let event = custom_col_event(EventKind::Delete, CustomCol {
                    id: p.id, ..Default::default()
                });
                Ok((event, vec![]))
            }
        }
