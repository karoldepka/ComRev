...
export async function recordOne(obs, resolved) {
  ...
  const browserSettings = {
    ...
    height,
    fps,
    // The page no longer plays any audio live (see "Sound events" above) —
    // nothing to route, and leaving this on could pick up incidental sounds
    // OBS itself makes no attempt to avoid.
    reroute_audio: false,
    shutdown: false,
  };
  ...
  // Captured before close() only for clarity — the events/music kind live in
  // startReadyServer's own closure, unaffected by the HTTP server shutting down.
...
async function main() {
  ...
  if (rawOpts.video === true || rawOpts.video === "all") {
    ...
    const langs = (rawOpts.lang || "pl,en").split(",").map((s) => s.trim()).filter(Boolean);
